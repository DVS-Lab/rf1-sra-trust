...
# Fit a linear model
lm_model <- lm(ecog_score ~ usi_sum, data)

# Extract the sample size
sample_size <- nrow(data)

# Create the scatterplot with a line of best fit and error bars
ggplot(data, aes(x = usi_sum, y = ecog_score)) +
  ...
  geom_text(x = max(data$usi_sum), y = min(data$ecog_score), label = paste("N =", sample_size), hjust = 1, vjust = 0) +
  labs(x = "USiDep",
       y = "ECog") +
  ...
  scale_x_continuous(limits = c(min(data$usi_sum), max(5))) +# Set x-axis limits # Set the font size for axis titles
  scale_y_continuous(limits = c(min(data$ecog_score), max(4)))
# Print the coefficient for the predictor variable "usi_sum" to console
Rvalue <- cor(data$usi_sum, data$ecog_score, use = "complete.obs")
sample_size <- sum(complete.cases(data$usi_sum, data$ecog_score))
summary(lm_model)
