...
# Fit a linear model
lm_model <- lm(fevs_sum ~ dudit_sum, data)

# Extract the sample size
sample_size <- nrow(data)

# Create the scatterplot with a line of best fit and error bars
ggplot(data, aes(x = dudit_sum, y = fevs_sum)) +
  ...
  geom_text(x = max(data$dudit_sum), y = min(data$fevs_sum), label = paste("N =", sample_size), hjust = 1, vjust = 0) +
  labs(x = "DUDIT",
       y = "FEVS") +
  ...
  scale_x_continuous(limits = c(min(data$dudit_sum), max(35))) +# Set x-axis limits # Set the font size for axis titles
...
Rvalue <- cor(data$dudit_sum, data$fevs_sum, use = "complete.obs")
sample_size <- sum(complete.cases(data$dudit_sum, data$fevs_sum))
summary(lm_model)
