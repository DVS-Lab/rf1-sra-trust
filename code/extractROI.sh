...
for TYPE in ppi_seed-VS; do
	for ROI in seed-VS; do
		...
		for model in ageoafem; do 
			...
			for copeinfo in "18 rec-def_SocClose" "17 def_SocClose"; do	
				# split copeinfo variable				
				set -- $copeinfo
				copenum=$1
				copename=$2
				MAINOUTPUT=${maindir}/derivatives/fsl/L3ppi_seed-VS/L3_model-${model}_task-trust_seed-VS_n${N}_flame1
