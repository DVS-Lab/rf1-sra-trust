...
# Run activation and one seed-PPI model sequentially within each L1 work unit.

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"

usage() {
    echo "Usage: run_L1_activation_ppi.sh --manifest FILE --seed NAME [--jobs N] [--log-dir DIR]" >&2
}
...
printf 'L1 activation+PPI batch plan: %d unit(s), %d job(s), model 1, seed=%s\n' \
    "${#units[@]}" "$jobs" "$seed"
...
wait_oldest() {
    ...
    if ! wait "$pid"; then
        echo "ERROR: failed L1 activation+PPI unit: $label${logfile:+ (log: $logfile)}" >&2
        failures=$((failures + 1))
    else
        echo "DONE: $label"
    fi
...
for unit in "${units[@]}"; do
    ...
    label="sub-${sub} ses-${ses} run-${run} act+PPI-${seed}"
    logfile=""
    [[ -n "$log_dir" ]] && logfile="${log_dir}/sub-${sub}_ses-${ses}_task-trust_run-${run}_act-plus-PPI-${seed}.log"
    echo "START: $label${logfile:+ (log: $logfile)}"
    if [[ -n "$logfile" ]]; then
        (
            echo 'PHASE: activation'
            bash "${SCRIPT_DIR}/L1stats.sh" "$sub" "$run" 0 --session "$ses" &&
            echo "PHASE: PPI seed-${seed}" &&
            bash "${SCRIPT_DIR}/L1stats.sh" "$sub" "$run" "$seed" --session "$ses"
        ) >"$logfile" 2>&1 &
    else
        (
            echo 'PHASE: activation'
